...
SUMMARIES_PATH = PROJECT_ROOT / "data" / "summary.csv"
...
def contains_all(summary: Any, expected_values: Iterable[Any]) -> bool:
    ...
    OR the summary explicitly states the category was excluded (e.g.
    "Account number(s) not included in this summary for brevity") --
    a stated, deliberate exclusion satisfies preservation just as well
    as including the value itself."""
    summary_text = normalize_for_contains(summary)
    if "not included in this summary" in summary_text:
        return True
...
def contains_amounts(summary: Any, expected_amounts: Iterable[Any]) -> bool:
    ...
    Also passes if the summary explicitly states amounts were excluded."""
    if "not included in this summary" in normalize_for_contains(summary):
        return True
